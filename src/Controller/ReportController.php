...
#[Route('/{_locale}/report', requirements: ['_locale' => '%app.locales%'])]
class ReportController extends AbstractController
{
    ...
    public function __construct(
        ...
        $this->leftMenu = $params->get('left_menu');
        $this->dataDef = $params->get('data_definition');
        $this->translator = $translator;
        $this->documentManager = $documentManager;
    }

    #[Route('/{provider}/{aspect}/{parameter}/{question}', name: 'report', requirements: [
        'provider' => '[^/]+',
        'aspect' => '[^/]+',
        'parameter' => '[^/]+',
        'question' => '[^/]+'
    ...
    public function report(
        ...
        string $_locale,
        string $provider,
        string $aspect = 'completeness',
        string $parameter = 'minimum',
        string $question = 'overview'
    ): Response {
        // Stel de locale in als deze niet aanwezig is
        if (!$_locale) {
        ...
        foreach ($providers as $prov) {
            if ($prov->getIdentifier() === $provider) {
        ...
        $functionCall = null;
        foreach ($this->leftMenu[$aspect]['parameters'] as $param) {
        ...
        if (!$functionCall || !method_exists($this, $functionCall)) {
            throw new NotFoundHttpException("Ongeldige rapportfunctie: $functionCall");
        }

        // Rapport ophalen via dynamische functieaanroep
